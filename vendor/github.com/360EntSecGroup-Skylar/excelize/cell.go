...
// ColLettersToIndex converts a character based column
...
// CellCoords returns the zero based cartesian
// coordinates from a cell name in Excel format, e.g. the cellIDString
// "A1" returns 0, 0 and the "B3" return 1, 2.
func CellCoords(axis string) (col, row int, error error) {
	letterPart := string(strings.Map(letterOnlyMapF, axis))
	row, err := strconv.Atoi(strings.Map(intOnlyMapF, axis))
	if err == nil {
		row -= 1 // Zero based
		col = ColLettersToIndex(letterPart)
	}
	return
}

// CellAddress returns the Excel format cell name that
// represents a pair of zero based cartesian coordinates.
func CellAddress(col, row int) string {
	return fmt.Sprintf("%s%d", ColIndexToLetters(col), row+1)
}
